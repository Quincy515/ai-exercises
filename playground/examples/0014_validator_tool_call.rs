...
impl UserInfo {
    ...
    fn parameters_schema() -> anyhow::Result<Value> {
        let mut schema = serde_json::to_value(schema_for!(UserInfo))?;
        schema
            .as_object_mut()
    ...
    fn tool() -> anyhow::Result<ChatCompletionTool> {
        Ok(ChatCompletionTool {
            function: FunctionObjectArgs::default()
                // 这里的 name 是发给模型看的工具名。
                // derive_name 让 UserInfo::name() 返回字符串 "UserInfo"，类似 Python 的 UserInfo.__name__。
                .name(Self::name())
    ...
    /// 从模型返回的 tool_call 中取出 JSON 参数，并转成 UserInfo。
    fn from_response(response: &CreateChatCompletionResponse) -> anyhow::Result<Self> {
        ...
        // serde_json::from_str 负责把 JSON 反序列化成 Rust struct。
        // 这里才真正得到一个 UserInfo 实例。
        let user_info: Self = serde_json::from_str(&tool_call.function.arguments)
            .with_context(|| format!("工具参数不是有效 JSON: {}", tool_call.function.arguments))?;

        // validator 根据 struct 字段上的 #[validate(...)] 做业务校验。
        // serde 负责“类型是否能解析”，validator 负责“值是否符合规则”。
        user_info.validate()?;
        Ok(user_info)
    }
}
